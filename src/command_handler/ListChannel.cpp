...
void ListChannelCommandHandler::execute(const std::vector<std::string> &params)
{
	if (params.size() == 0)
	{
		...
		_server.replyToClient(_client.getFd(), MSG_LIST(_client.get_prefix(), channelList));
		return;
	}
	...
	_server.replyToClient(_client.getFd(), RPL_NAMREPLY(_client.get_prefix(),
			channelName, channel->getMemberList(_server)));
	_server.replyToClient(_client.getFd(), RPL_ENDOFNAMES(_client.get_prefix(),
			channelName));
}